<template>
    <div v-if="this.$store.state.modal.showActionModal" class="fixed z-40">
        <div v-if="this.$store.state.post.postAction"
            class="fixed top-0 bottom-0 left-0 right-0 bg-black/[0.2] flex items-center justify-center"
            @click="hiddenModal()">
...
export default {
    setup() {
        ...
        const handleDeleteComment = async (commentId) => {
            ...
            socket?.emit('deleteComment', store.state.auth.user?._id);
        ...
        const handleReport = async (postId, commentId, userId, optionId) => {
            ...
            if (response.status == 201) {
                ...
                if (postId && route.name == 'DetailPost') {
                    router.back();
                }
