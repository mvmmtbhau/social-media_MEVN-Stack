<template>
    <div v-if="this.$store.state.modal.showActionModal" class="fixed z-40">
        <div class="fixed top-0 bottom-0 left-0 right-0 bg-black/[0.2] flex items-center justify-center"
            @click="hiddenModal()">
            <div v-if="!showOptionReport">
                <div v-if="this.$store.state.auth.user?._id != this.$store.state.post.postAction?.owner._id"
                    ...
                    <div class="relative flex gap-3 px-4 py-3 items-center w-full cursor-pointer active:bg-gray-200 transition-all duration-100
