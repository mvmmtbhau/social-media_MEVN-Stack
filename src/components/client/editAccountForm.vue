<template>
    <Form @submit="handleUpdate" :validation-schema="editFormSchema">
        <div class="flex items-center ml-28 gap-6 pt-5 ">
            <div v-if="avatar" id="show_new_avatar">
                
            </div>
            <img v-else-if="this.$store.state.auth.user?.avatar"
            ...
            <Field hidden type="file" v-model="avatar" @change="showImage" name="avatar" id="avatar" />
            <label for="avatar" class="font-bold text-sm text-cyan-500 cursor-pointer hover:text-black">
...
export default {
    ...
    setup() {
        ...
        const showImage = () => {
            const isExisted = document.getElementById('new_avatar');
            if(isExisted) isExisted.remove();

            let divShow = document.getElementById('show_new_avatar');

            let imgEle = document.createElement('img');
            imgEle.src = URL.createObjectURL(avatar.value);
            imgEle.id = 'new_avatar';
            imgEle.className = 'w-10 h-10 rounded-full';

            divShow.appendChild(imgEle);
        }
